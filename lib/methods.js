...
/**
Adds the default options applied to all future requests, inheriting the existing defaults.  Any passed defaults override what was in there.
@param {Object} [defaults] - default request options, see the popular {@link https://github.com/request/request#requestoptions-callback request library} for options
@alias module:chakram.addRequestDefaults
 */
exports.addRequestDefaults = function(defaults) {
    defaultsJar = defaults.jar || defaultsJar; // We want this copied also if it's undefined
    defaultedRequestObj = (defaultedRequestObj || request).defaults(defaults);
};
